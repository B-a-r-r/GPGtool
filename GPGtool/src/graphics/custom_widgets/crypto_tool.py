...
class Crypto_tool(ctk.CTkFrame):
    ...
    _result_menu: ctk.CTkOptionMenu
    ...
    def __init__(self, **kwargs)->None:
        ...
        self._main_frame = ctk.CTkScrollableFrame(master=self._master, orientation="horizontal", border_color='white', border_width=2)
        
        self._input_frame = ctk.CTkFrame(master=self._main_frame, border_color='white', border_width=2)
        
        self._result_frame = ctk.CTkFrame(master=self._main_frame, border_color='white', border_width=2)
        ...
        self._entry_menu = ctk.CTkOptionMenu(master=self._input_frame,
                                            variable=ctk.StringVar(value="Select source"),
                                            values=self._FORMAT,
                                            dynamic_resizing=True,
                                            command=self.entry_menu_changed
        )
        self._text_entry = ctk.CTkTextbox(master=self._input_frame, 
                                         width=400, 
                                         height=300, 
                                         cursor="xterm", 
                                         text_color="grey")
        
        self.passphrase_entry = ctk.CTkEntry(master=self._input_frame, 
                                             width=350, 
                                             placeholder_text="Passphrase...")
        
        self._key_entry = ctk.CTkEntry(master=self._input_frame, width=350, placeholder_text="Key...")
    ...
    def display_configuration(self)->None:
        ...
        self._main_frame.grid(column=1,
                             row=0,
                             columnspan=self._master.grid_size()[0]-1, 
        ...
        self._result_frame.grid_columnconfigure((0, 2), weight=1)
    ...
    def refresh_layout(self, event=None)->None:
        ...
        match self._selected_mode:
            ...
            case "Encryption":
                self._entry_menu.grid(column=0, row=0, pady=5)
                
                if (not event == "text_entry_clicked"):
                    self.text_entry_placeholder(mode="encrypt")
                self._text_entry.grid(column=1, row=0, rowspan=2, pady=5, ipady=20, sticky="e")
                
                self._key_entry.configure(placeholder_text="Public key...")
                self._key_entry.grid(column=2, row=2, pady=5)
                
                self._text_result.grid(column=1, row=0, pady=10)
    ...
    def entry_menu_changed(self, event=None)->None:
        ...
        if (self._entry_menu.get() not in [self._selected_entry_format, 'Select source']):
            self.switch_entry_format(self._entry_menu.get())
    
    def switch_entry_format(self, new_format:str)->None:
        """
        Changes the selected entry _FORMAT, disposing all the components, then refreshs the display.
        """
        self._selected_entry_format = new_format
        self.dispose_all(inner_command=True)
        self.refresh_layout()
